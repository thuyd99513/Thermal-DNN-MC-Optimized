...
from utilities_autocorr import num_dof, print_q, print_qq, print_qq_in, print_qq_out, plot_ave_autocorr_S, plot_ave_autocorr_J
...
def autocorr_log_ave_over_sample(L=L, M=M, N=N, tot_steps=tot_steps, beta=beta, N_in=N_in, N_out=N_out, tw=tw, n_replica=n_replica,init_list=init_list, inner_representation=False, autocorr_statistics=False, X_index=2):
    ...
    #========================================================================
    res_J=np.load('{}/{}/autocorr_J_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,X_index,tw,L,N,beta,tot_steps))
    res_J_in=np.load('{}/{}/autocorr_J_in_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,X_index,tw,L,N,beta,tot_steps))
    res_J_out=np.load('{}/{}/autocorr_J_out_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,X_index,tw,L,N,beta,tot_steps))
    res_S=np.load('{}/{}/autocorr_S_{:s}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,X_index,tw,L,M,N,beta,tot_steps))
    ...
    for j,init in enumerate(init_list):     
        ...
        temp_res_J_in[j] = np.load('{}/{}/autocorr_J_in_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir_,stamp2,stamp2,init,tw,L,N,beta,tot_steps))
        temp_res_J_out[j] = np.load('{}/{}/autocorr_J_out_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir_,stamp2,stamp2,init,tw,L,N,beta,tot_steps))
    ...
    np.save('{}/{}/autocorr_J_in_ave_over_weight_sample_{:s}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,tw,L,N,beta,tot_steps),res_J_in_ave_over_sample)
    np.save('{}/{}/autocorr_J_out_ave_over_weight_sample_{:s}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,tw,L,N,beta,tot_steps),res_J_out_ave_over_sample)
    ...
    np.save('{}/{}/autocorr_std_J_in_ave_over_weight_sample_{:s}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,tw,L,N,beta,tot_steps),std_J_in_ave_over_sample)
    np.save('{}/{}/autocorr_std_J_out_ave_over_weight_sample_{:s}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,tw,L,N,beta,tot_steps),std_J_out_ave_over_sample)
    ...
    if autocorr_statistics:
        autocorr_tstar_S = np.zeros((n_init, n_replica, L-1))
        autocorr_tstar_J = np.zeros((n_init, n_replica, L-2)) # Maybe for J_in and J_out can do the same thing
        for j,init in enumerate(init_list):     
            data_dir_ = '../../ir_hf_L{:d}_M{:d}_N{:d}_sample{:d}_mp_tw{:d}/data1'.format(L,M,N,init,tw)
            timestamp_list2 = list_only_naked_dir(data_dir_) # There is only one directory.
            stamp2 = timestamp_list2[0]
            print(autocorr_tstar_S.shape)
            print(autocorr_tstar_J.shape)
            autocorr_tstar_S[j] = np.load('{}/{}/autocorr_S_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir_,stamp2,stamp2,init,tw,L,M,N,beta,tot_steps))
            autocorr_tstar_J[j] = np.load('{}/{}/autocorr_J_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir_,stamp2,stamp2,init,tw,L,N,beta,tot_steps))
        n_init_replica = n_init * n_replica
        autocorr_tstar_S = np.reshape(autocorr_tstar_S, (n_init_replica, L-1))      
        autocorr_tstar_J = np.reshape(autocorr_tstar_J, (n_init_replica, L-2)) 
        autocorr_tstar_S_T = autocorr_tstar_S.T        
        autocorr_tstar_J_T = autocorr_tstar_J.T
        ...
        for l in range(L-1):
            file_autocorr_tstar = open("{}/autocorr_S_tstart_tw{:d}_L{:d}_M{:d}_N{:d}_step{:d}_l{:d}.dat".format(data_dir,tw,L,M,N,tot_steps,l),"w")
            file_autocorr_tstar.write("# autocorr_S\n")
            for j in range(n_init_replica): 
                file_autocorr_tstar.write("{}\n".format( autocorr_tstar_S_T[l][j]) )
            file_autocorr_tstar.close()
...
if __name__ == '__main__':
    ...
    for i in range(NUM_CORES):
        para_tuple.append((L,M,N,tot_steps, beta, N_in, N_out, tw, n_replica, init_list, False, X_index)) 
