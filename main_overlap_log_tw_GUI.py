...
from utilities_overlap import overlap_J_hidden, overlap_J_in, overlap_J_out, overlap_S, plot_overlap_J_hidden, plot_overlap_S,\
     plot_test_delta_e_bond, plot_test_delta_e_node, plot_test_delta_e_bond_node_hidden, num_dof,\
     num_dof_inner_representation
from Network import qq, step_list_v2, generate_traj_sampling_num
...
def overlap_log_tw(L=L, M=M, N=N, tot_steps=tot_steps, beta=beta, N_in=N_in, N_out=N_out, tw=tw, sample_index=sample_index, n_replica=n_replica, overlap_statistics=False, inner_representation=False):
    ...
    #method: match pattern with glob
    #==============================================
    import glob
    i = 0
    ...
    for term in prefixed:
        #The file names have the format like "J_hidden_1629046894_sample2_*", thus use the following comd. to extract the timestamp, eg, 1629046894. 
        str_replica_index_list.append(term.split("_",3)[2])

    res_overlap_1 = np.zeros((n_pairs,num_hidden_bond_layers,tot_steps_))
    res_overlap_1_in = np.zeros((n_pairs,tot_steps_))
    res_overlap_1_out = np.zeros((n_pairs,tot_steps_))
    ...
    for ol_index, str_replicas in enumerate(tuple_replicas):
        J_in_traj = np.load('{}/{}/J_in_{:s}_sample{:d}_tw{:d}_N{:d}_N_in{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[0],sample_index,tw,N,N_in,beta,tot_steps))
        J0_in_traj = np.load('{}/{}/J_in_{:s}_sample{:d}_tw{:d}_N{:d}_N_in{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[1],sample_index,tw,N,N_in,beta,tot_steps))
        J_out_traj = np.load('{}/{}/J_out_{:s}_sample{:d}_tw{:d}_N_out{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[0],sample_index,tw,N_out,N,beta,tot_steps))
        J0_out_traj = np.load('{}/{}/J_out_{:s}_sample{:d}_tw{:d}_N_out{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[1],sample_index,tw,N_out,N,beta,tot_steps))
        J_traj = np.load('{}/{}/J_hidden_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[0],sample_index,tw,L,N,beta,tot_steps))
        J0_traj = np.load('{}/{}/J_hidden_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[1],sample_index,tw,L,N,beta,tot_steps))
        S_traj = np.load('{}/{}/S_{}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[0],sample_index,tw,L,M,N,beta,tot_steps))
        S0_traj = np.load('{}/{}/S_{}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,str_replicas[1],sample_index,tw,L,M,N,beta,tot_steps))
        res_overlap_1[ol_index] = overlap_J_hidden(J_traj, J0_traj)
        res_overlap_1_in[ol_index] = overlap_J_in(J_in_traj, J0_in_traj)
        res_overlap_1_out[ol_index] = overlap_J_out(J_out_traj, J0_out_traj)
    ...
    mean_ol_1_in = np.mean(res_overlap_1_in,axis=0)
    mean_ol_1_out = np.mean(res_overlap_1_out,axis=0)
    mean_ol_2 = np.mean(res_overlap_2,axis=0)
    np.save('{}/{}/overlap_J_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,N,beta,tot_steps),mean_ol_1)
    np.save('{}/{}/overlap_J_in_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,N,beta,tot_steps),mean_ol_1_in)
    np.save('{}/{}/overlap_J_out_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,timestamp_list[i],sample_index,tw,L,N,beta,tot_steps),mean_ol_1_out)
    np.save('{}/{}/overlap_S_{:s}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,timestamp_list[i],sample_index,tw,L,M,N,beta,tot_steps),mean_ol_2)
    if overlap_statistics:
        overlap_J_tstar = res_overlap_1[:,:,-1]
        overlap_J_in_tstar = res_overlap_1_in[:,-1]
        overlap_J_out_tstar = res_overlap_1_out[:,-1]
        overlap_S_tstar = res_overlap_2[:,:,-1]
        np.save('{}/{:s}/overlap_J_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,N,beta,tot_steps), overlap_J_tstar)
        np.save('{}/{:s}/overlap_J_in_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,N,beta,tot_steps), overlap_J_in_tstar)
        np.save('{}/{:s}/overlap_J_out_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,N,beta,tot_steps), overlap_J_out_tstar)
        np.save('{}/{:s}/overlap_S_tstar_{:s}_sample{:d}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,stamp,stamp,sample_index,tw,L,M,N,beta,tot_steps), overlap_S_tstar)
    #=====================================================
    ...
    #=====================================================
    plot_overlap_J_hidden(mean_ol_1,timestamp_list[i],sample_index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list_v2)
    plot_overlap_S(mean_ol_2,timestamp_list[i],sample_index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list_v2) 
...
if __name__ == '__main__':
    ...
    M,L,N,beta,tot_steps,X_index,n_replica = args.M,args.L,args.N,args.B,args.S,args.C,args.R
    ...
    NUM_CORES = n_replica # THIS is A PARAMETER. It denotes how many cores you plan to use. Here, we only use one waiting time, therefore, we let it be n_replca. 
    print(str(NUM_CORES) + " cores are used.")
    
    # Define parameters for each process, and store each set of input as a value of a dictionary dic_para.
    dic_para = {}
    for i in range(NUM_CORES):
        dic_para["para{}".format(i)] = (L,M,N,tot_steps, beta, N_in, N_out, 1024, X_index, n_replica, True, False)
    for key, value in dic_para.items():
        print("Now start process ({}).".format(key))
        mp.Process(target=overlap_log_tw, args=value).start() #start now
        sleep(1)
