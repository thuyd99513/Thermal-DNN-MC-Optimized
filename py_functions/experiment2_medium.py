#!/usr/bin/env python3
"""
experiment2_medium.py - 中等规模实验二：全局训练动态 (Figure 2)

使用真实 MNIST 数据，中等规模参数以平衡速度和结果质量。
"""
...
def run_experiment(X_train, y_train, X_test, y_test, N, L, beta, 
    ...
    print(f"E(0)={E0:.2f}, A_test(0)={compute_test_accuracy(net_a, X_test, y_test):.4f}")
    
    time_pts, E_ratio, A_train, A_test = [0], [1.0], [compute_train_accuracy(net_a)], [compute_test_accuracy(net_a, X_test, y_test)]
    ...
    for step in range(1, mc_steps + 1):
        ...
        if step % log_interval == 0 or step == 1:
            ...
            A_train.append(compute_train_accuracy(net_a))
            A_test.append(compute_test_accuracy(net_a, X_test, y_test))
            
            for tw in t_w_list:
                if step == tw: S_tw[tw] = net_a.S.copy()
                if S_tw[tw] is not None and step >= tw:
                    ...
                    for l in range(L-1):
                        ...
                        layer_autocorr[tw][l]['c'].append(np.mean(net_a.S[:, l, :] * S_tw[tw][:, l, :]))
            
            for l in range(L-1):
                layer_overlap[l]['t'].append(step)
                layer_overlap[l]['q'].append(np.mean(net_a.S[:, l, :] * net_b.S[:, l, :]))
...
def plot_results(results_list, output_dir):
    ...
    ax.set_xlabel('t (MC steps)'); ax.set_ylabel('E(t)/E(0)'); ax.set_title('(a) Energy decay')
    ...
    ax.set_xlabel('t'); ax.set_ylabel('A_train(t)'); ax.set_title('(b) Training accuracy')
    ...
    ax.set_xlabel('t'); ax.set_ylabel('A_test(t)'); ax.set_title('(c) Test accuracy')
    ax.legend(fontsize=9); ax.grid(True, alpha=0.3); ax.set_ylim(0, 1.05)
    ...
    ax.set_xlabel('ln α'); ax.set_ylabel('E*/E(0)', color='b'); ax2.set_ylabel('A*_test', color='r')
    ax.set_title('(d) Final values'); ax.legend(handles=[l1, l2], fontsize=9); ax.grid(True, alpha=0.3)
    ...
    if results_list:
        ...
        for i, tw in enumerate(r['t_w_list']):
            ...
            if d['t']: ax.semilogx(d['t'], d['c'], 'o-', ms=3, color=tw_c[i], label=f"t_w={tw}")
    ax.set_xlabel('t'); ax.set_ylabel('c(t, t_w)'); ax.set_title('(e) Spin autocorrelation')
    ...
    if results_list:
        ...
        for i, tw in enumerate(r['t_w_list']):
            d = r['overlap_data'][tw]
            if d['t']: ax.semilogx(d['t'], d['q'], 'o-', ms=3, color=tw_c[i], label=f"t_w={tw}")
    ax.set_xlabel('t'); ax.set_ylabel('q(t, t_w)'); ax.set_title('(f) Replica overlap')
    ax.legend(fontsize=9); ax.grid(True, alpha=0.3); ax.set_ylim(-0.5, 1.1)
    ...
    if results_list:
        ...
        if tw:
            ...
            for l in range(L-1):
                ...
                if d['t']: ax.semilogx(d['t'], d['c'], 'o-', ms=2, color=lc[l], label=f"l={l+1}")
    ax.set_xlabel('t'); ax.set_ylabel('c_l(t, t_w)'); ax.set_title('(g) Layer autocorrelation')
    ax.legend(fontsize=7, ncol=2); ax.grid(True, alpha=0.3); ax.set_ylim(-0.5, 1.1)
    ...
    if results_list:
        ...
        for l in range(L-1):
            ...
            if d['t']: ax.semilogx(d['t'], d['q'], 'o-', ms=2, color=lc[l], label=f"l={l+1}")
    ax.set_xlabel('t'); ax.set_ylabel('q_l(t)'); ax.set_title('(h) Layer overlap')
    ax.legend(fontsize=7, ncol=2); ax.grid(True, alpha=0.3); ax.set_ylim(-0.5, 1.1)
...
def main():
    print("=" * 60)
    print("实验二：全局训练动态 (Figure 2) - 中等规模")
    print("=" * 60)
    ...
    for r in results_list:
        print(f"{r['ln_alpha']:>8.2f} | {r['energy_ratio'][-1]:>12.4e} | {r['accuracy_train'][-1]:>10.4f} | {r['accuracy_test'][-1]:>10.4f}")
