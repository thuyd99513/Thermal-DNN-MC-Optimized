"""
run_experiment1_fast.py - 快速版本的实验一复现
...
def run_replica_simulation(M, N, L, beta, mc_steps, num_replicas=2, 
                           N_in=784, N_out=2, seed=None, verbose=True):
    """运行多副本模拟并计算重叠参数"""
    ln_alpha = np.log(M / N)
    
    if verbose:
        print(f"\n{'='*60}")
        print(f"副本模拟: M={M}, N={N}, L={L}, ln(α)={ln_alpha:.2f}")
        print(f"MC steps: {mc_steps}, 副本数: {num_replicas}")
        print(f"{'='*60}")
    ...
    for step in range(mc_steps + 1):
        ...
        # 进度报告
        if verbose and step > 0 and step % report_interval == 0:
            ...
            print(f"  Step {step}/{mc_steps} ({100*step/mc_steps:.0f}%), "
                  f"<q>={q_mean:.4f}, {rate:.1f} steps/s, ETA: {eta/60:.1f}min")
...
def main():
    ...
    mc_steps = 80000  # 8万步，每个配置约5-10分钟
    ...
    for cfg in configs:
        ...
        print(f"    M={cfg['M']}, N={cfg['N']}, ln(α)={ln_alpha:.2f}")
    ...
    for i, cfg in enumerate(configs):
        ...
        # 打印当前结果
        print(f"\n层重叠参数 q_l* (ln α = {ln_alpha:.2f}):")
        for l in range(L - 1):
            phase = "固态" if q_l_star[l] > 1/np.e else "液态"
            print(f"    层 {l+1}: q_l* = {q_l_star[l]:.4f} ({phase})")
    
    total_time = time.time() - total_start
    print(f"\n{'='*60}")
    print(f"所有模拟完成，总耗时: {total_time/60:.1f} 分钟")
    print(f"{'='*60}")
    
    # 绘制相图
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              '..', 'reports', 'experiment1_fast')
