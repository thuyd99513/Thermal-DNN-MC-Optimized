...
from utilities_overlap import overlap_J_hidden, overlap_S, plot_overlap_J_hidden, plot_overlap_S, \
     plot_test_delta_e_bond, plot_test_delta_e_node, plot_test_delta_e_bond_node_hidden, plot_acc 
...
def acc_tw(L=L, M=M, N=N, tot_steps=tot_steps, beta=beta, N_in=N_in, N_out=N_out, tw=tw, sample_index=sample_index, n_replica=n_replica):
    ...
    score0 = np.zeros((n_replica,tot_steps_))
    score1 = np.zeros((n_replica,tot_steps_))
    score = np.zeros((n_replica,tot_steps_))
    acc = np.zeros((n_replica,tot_steps_))
    mean_acc = np.zeros(tot_steps_)
    acc0 = np.zeros((n_replica,tot_steps_))
    mean_acc0 = np.zeros(tot_steps_)
    acc1 = np.zeros((n_replica,tot_steps_))
    mean_acc1 = np.zeros(tot_steps_)
    r_index = 0
    ...
    for r_index, str_replicas in enumerate(str_replica_index_list):
        ...
        for step in range(tot_steps_):
            ...
            print("Predicted 0-samples: {}\n, {}".format(S_out_predict_traj[step,:], S_out[:]))
            for mu in range(int(M/2)):
                print("Predicted: {}".format((S_out_predict_traj[step,mu] == S_out[mu]).all()))
                score0[r_index,step] += (S_out_predict_traj[step,mu] == S_out[mu]).all()
            acc0[r_index,step] = score0[r_index,step]/(M/2)
            print("ACC for a replica: {}".format(acc0[r_index,step]))

            print("Predicted 1-samples: {}\n, {}".format(S_out_predict_traj[step,:], S_out[:]))
            for mu in range(int(M/2),M):
                print("Predicted: {}".format((S_out_predict_traj[step,mu] == S_out[mu]).all()))
                score1[r_index,step] += (S_out_predict_traj[step,mu] == S_out[mu]).all()

            acc1[r_index,step] = score1[r_index,step]/(M/2)
            print("ACC1 for a replica: {}".format(acc1[r_index,step]))

            print("Predicted 0-1-samples: {}\n, {}".format(S_out_predict_traj[step,:], S_out[:]))
            for mu in range(M):
                print("Predicted: {}".format((S_out_predict_traj[step,mu] == S_out[mu]).all()))
    ...
    mean_acc0 = np.mean(acc0,axis=0)
    mean_acc1 = np.mean(acc1,axis=0)
    mean_acc = np.mean(acc,axis=0)
    print("All ACC over replica for all steps: {}".format(mean_acc))
    plot_acc(mean_acc0, mean_acc1,mean_acc,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list_v2)
