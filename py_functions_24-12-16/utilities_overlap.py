...
#def pt_acc(acc0, acc1,acc,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
...
#    pt(time_arr[ex:],acc0[ex:],color=color_list[0],marker='.',label="0")
#    #ax.plot(time_arr[ex:],acc1[ex:],color=color_list[1],marker='*',label="1")
#    #ax.plot(time_arr[ex:],acc[ex:],color=color_list[2],marker='o',label="all")
...
def plot_acc(acc0, acc1,acc,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    fig = plt.figure()
    ...
    color_list = get_N_HexCol(num_color)
    ax = fig.add_subplot(111) # add_subplot() adds an axes to a figure, it returns a (subclass of a) matplotlib.axes.Axes object.
    ax.plot(time_arr[ex:],acc0[ex:],color=color_list[0],marker='.',label="0")
    ax.plot(time_arr[ex:],acc1[ex:],color=color_list[1],marker='*',label="1")
    ax.plot(time_arr[ex:],acc[ex:],color=color_list[2],marker='o',label="all")
    plt.legend(loc="lower left")
    plt.xlabel("t")
    plt.ylabel(r"acc$(t)$")
    ax.set_title(r"$t_w={:d};L={:d};M={:d};N={:d};\beta={:3.1f}$".format(tw,L,M,N,beta))
    plt.savefig("../imag/acc_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}_log.eps".format(tw,L,M,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/acc_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}_log.png".format(tw,L,M,N,beta,tot_steps),format='png')
    plt.savefig("../imag/acc_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}_log.pdf".format(tw,L,M,N,beta,tot_steps),format='pdf')

def plot_overlap_S(q,index,init,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    fig = plt.figure()
...
def print_q(q,std,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list,inner_representation=False):
    """overlap_J is the average one over different init's."""
    ...
    for l in l_S_index_list[:L-1]: 
        f = open("../data1/q_L{:d}_M{:d}_N{:d}_tw{:d}_l{:d}.dat".format(L,M,N,tw,l),"w")
        f.write("# time     q       std\n")
        for i in range(tot_steps_): 
            f.write("{:7.4f}  {:7.6f}  {:7.6f}\n".format(time_arr[i], q[l][i], std[l][i])) 
        f.close()
def print_qq(Q,std,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list,inner_representation=False):
    ...
    time_arr = np.array(step_list[:tot_steps_])/num
    #Print the data Q(l,t)
    for l in l_S_index_list[:L-2]: 
        f = open("../data1/QQ_L{:d}_M{:d}_N{:d}_tw{:d}_l{:d}.dat".format(L,M,N,tw,l),"w")
        f.write("# time     Q    std\n")
        for i in range(tot_steps_): 
            f.write("{:7.4f}  {:7.6f}  {:7.6f} \n".format(time_arr[i], Q[l][i], std[l][i] )) 
        f.close()
def print_qq_in(Q,std,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list,inner_representation=False):
    if inner_representation:
        num = num_dof_inner_representation(L,M,N,N_in,N_out)
    else:
        num = num_dof(L,M,N,N_in,N_out)
    time_arr = np.array(step_list[:tot_steps_])/num
    f = open("../data1/QQ_in_L{:d}_M{:d}_N{:d}_tw{:d}.dat".format(L,M,N,tw),"w")
    f.write("# time     Q     std\n")
    for i in range(tot_steps_): 
        f.write("{:7.4f}  {:7.6f} {:7.6f}\n".format(time_arr[i], Q[i], std[i])) 
    f.close()
def print_qq_out(Q,std,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list,inner_representation=False):
    if inner_representation:
        num = num_dof_inner_representation(L,M,N,N_in,N_out)
    else:
        num = num_dof(L,M,N,N_in,N_out)
    time_arr = np.array(step_list[:tot_steps_])/num
    f = open("../data1/QQ_out_L{:d}_M{:d}_N{:d}_tw{:d}.dat".format(L,M,N,tw),"w")
    f.write("# time     Q    std\n")
    for i in range(tot_steps_): 
        f.write("{:7.4f}  {:7.6f} {:7.6f}\n".format(time_arr[i], Q[i], std[i])) 
    f.close()
def plot_ave_overlap_S(q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    """overlap_S is the average one over different init's."""
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    ...
    #Print the ref line for fitting 
    #f = open("../data1/Ref.dat","w") #R7
    #for i in range(tot_steps_):  #R8
    #    f.write("{:8.4f}  {:7.6f}\n".format(time_arr[i], q[x+7][i]))  #R9
    #f.close() #R10
    #Print the data q(l,t)
    for l in l_S_list: 
        f = open("../data1/q_L{:d}_M{:d}_N{:d}_l{:d}.dat".format(L,M,N,l),"w")
        f.write("# time     q\n")
        for i in range(tot_steps_): 
...
def plot_ave_overlap_S_loglog(q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    """overlap_S is the average one over different init's."""
    num = num_dof(L,M,N,N_in,N_out)
    #num = M*L*N + L*N*N #Maybe this formula is also possible sometime.
    ex = 0
    ...
    plt.ylabel(r"$q(t,l)$")
    ax.set_title("tw={:d};L={:d};M={:d};N={:d}; beta={:3.1f}".format(tw,L,M,N,beta))
    plt.savefig("../imag/Ave_overlap_S_{:s}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}_loglog.eps".format(index,tw,L,M,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Ave_overlap_S_{:s}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}_loglog.pdf".format(index,tw,L,M,N,beta,tot_steps),format='pdf')
...
def plot_ave_overlap_S_linear(q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    """overlap_S is the average one over different init's."""
    num = num_dof(L,M,N,N_in,N_out)
    #num = M*L*N + L*N*N #Maybe this formula is also possible sometime.
    ex = 0
    ...
    plt.ylabel(r"$q(t,l)$")
    ax.set_title("tw={:d};L={:d};M={:d};N={:d}; beta={:3.1f}".format(tw,L,M,N,beta))
    plt.savefig("../imag/Ave_overlap_S_{:s}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}.eps".format(index,tw,L,M,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Ave_overlap_S_{:s}_tw{:d}_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}.pdf".format(index,tw,L,M,N,beta,tot_steps),format='pdf')
...
def overlap_J(J_traj,J0_traj,J_in_traj,J0_in_traj,J_out_traj,J0_out_traj):
    ...
    for l in range(0,L):
        if l == 0:
            ...
            for t in range(T):
                res[l][t] = np.sum(J_traj[t,l0,:,:] * J0_traj[t,l0,:,:] )/N_SQ
                res[l][t] = round(res[l][t],5)
                #print("l={}".format(l))
    ...
    for l in range(L_hidden):
        for t in range(T):
            for n2 in range(N):
                ...
                #print(res[n2][l][t])
    return res
def overlap_J_in(J_X_traj,J0_X_traj):
    '''overlap for J_in_traj and J0_in_traj Q(t,l).'''
    T = J_X_traj.shape[0]
    print("shape of J_X_traj")
    print(J_X_traj.shape)
    Nx= J_X_traj.shape[-1]
    N2= J_X_traj.shape[-2]
    res_temp = np.zeros((Nx,T),dtype='float32')
    res = np.zeros((Nx,T),dtype='float32')
    for t in range(T):
        for n2 in range(N2):
            res_temp[n2][t] = np.sum(J_X_traj[t,n2,:] * J0_X_traj[t,n2,:] )/N2
            res_temp[n2][t] = round(res_temp[n2][t],5)
    res = np.mean(res_temp,axis=0)
    return res
def overlap_J_out(J_X_traj,J0_X_traj):
    '''overlap for J_out_traj and J0_out_traj Q(t,l).'''
    T = J_X_traj.shape[0]
    print("shape of J_X_traj")
    print(J_X_traj.shape)
    #L_in = 1 and  L_out = 1
    # Shape of J_in_traj is (Y, N, 784) 
    # Shape of J_out_traj is (Y, N_out, N) 
    Nx= J_X_traj.shape[-1]
    N2= J_X_traj.shape[-2]
    res_temp = np.zeros((Nx,T),dtype='float32')
    res = np.zeros((Nx,T),dtype='float32')
...
def overlap_J_hidden(J_traj,J0_traj):
    ...
    res = np.zeros((L_hidden,T,N),dtype='float32')
    for l in range(L_hidden):
        for t in range(T):
            for n2 in range(N):
                res_temp[l][t][n2] = np.sum(J_traj[t,l,n2,:] * J0_traj[t,l,n2,:] )/N
                res_temp[l][t][n2] = round(res_temp[l][t][n2],5)
                #print("res_temp[{}][{}][{}]: Q_hidden".format(l,t,n2))
                #print(res_temp[l][t][n2])
    res = np.mean(res_temp,axis=2)
    return res
def overlap_J_hidden_v2(J_traj,J0_traj):
    '''overlap for J_traj and J0_traj Q(t,l).'''
    T = J_traj.shape[0]
    L_hidden = J_traj.shape[1] 
    N = J_traj.shape[-1]
    ...
    for l in range(L_hidden):
        for t in range(T):
            res[l][t] = np.sum(J_traj[t,l,:,:] * J0_traj[t,l,:,:] )/N_SQ
            res[l][t] = round(res[l][t],5)
            #print("res[{}][{}]: Q_hidden".format(l,t))
            #print(res[l][t])
    return res
    ...
    for l in range(0,L):
        for t in range(T):
            for n2 in range(N):
                res_temp[l][t][n2] = np.sum(J_traj[t,l,n2,:] * J0_traj[t,l,n2,:])/N
                res_temp[l][t][n2] = round(res_temp[l][t][n2],5)
                #print("res_temp[{}][{}][{}]: Q".format(l,t,n2))
            #print(res_temp[l][t][n2])
    res = np.mean(res_temp,axis=2)
    return res
...
def overlap_S(S_traj,S0_traj):
    ...
    for l in range(0,L): # we do not need the spins in the input layer
        for t in range(T):
            res[l][t] = np.sum(S_traj[t,:,l,:] * S0_traj[t,:,l,:])/NM
            #print("res[{}][{}]: q".format(l,t))
            #print(res[l][t])
    return res

def plot_overlap_J_tw(ol0,ol1,ol2,ol3,ol4,ol5,ol6,timestamp,init,l_index,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    ...
    plt.savefig("../imag/Overlap_J_hidden_{:s}_init{:d}_l{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_log.pdf".format(timestamp,init,l_index,L,N,beta,tot_steps),format='pdf')

def plot_overlap_S_tw(ol0,ol1,ol2,ol3,ol4,ol5,ol6,ol7,index,init,l_index,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,tw_list,step_list):
    ...
    plt.savefig("../imag/Overlap_S_{:s}_init{:d}_l{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_log.pdf".format(index,init,l_index,L,N,beta,tot_steps),format='pdf')

def plot_overlap_J_tw_ave_over_init(ol0,ol1,ol2,ol3,ol4,ol5,ol6,timestamp,l_index,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,tw_list,step_list):
...
def plot_overlap_S_tw_ave_over_init(ol0,ol1,ol2,ol3,ol4,ol5,ol6,ol7,ol8,index,l_index,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,tw_list,step_list):
...
def plot_overlap_J(Q,index,sample,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps):
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    fig = plt.figure()
    ...
    plt.savefig("../imag/Overlap_J_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_log.eps".format(index,sample,tw,L,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Overlap_J_{:s}_sample{:d}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_log.pdf".format(index,sample,tw,L,N,beta,tot_steps),format='pdf')
...
def num_dof(L,M,N,N_in,N_out):
    ...
    num = num_variables + num_bonds
    return num 
def plot_overlap_J_hidden(Q,index,sample,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    fig = plt.figure()
    ...
    plt.savefig("../imag/Overlap_J_hidden_{:s}_init{:d}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_log.pdf".format(index,sample,tw,L,N,beta,tot_steps),format='pdf')
...
def plot_ave_overlap_J(Q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
...
def plot_ave_overlap_J_loglog(Q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    ...
    plt.ylabel(r"$Q(t,l)$")
    #ax.set_title("tw={:d};L={:d};M={:d};N={:d}; beta={:3.1f}".format(tw,L,M,N,beta))
    plt.savefig("../imag/Ave_overlap_J_hidden_{:s}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_loglog.eps".format(index,tw,L,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Ave_overlap_J_hidden_{:s}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}_loglog.pdf".format(index,tw,L,N,beta,tot_steps),format='pdf')
def plot_ave_overlap_J_linear(Q,index,tw,L,M,N,N_in,N_out,beta,tot_steps_,tot_steps,step_list):
    """The argument overlap_J is the average one over different init's."""
    num = num_dof(L,M,N,N_in,N_out)
    ex = 0
    time_arr = np.array(step_list[:tot_steps_])/num
    ...
    plt.ylabel(r"$Q(t,l)$")
    #ax.set_title("tw={:d};L={:d};M={:d};N={:d}; beta={:3.1f}".format(tw,L,M,N,beta))
    plt.savefig("../imag/Ave_overlap_J_hidden_{:s}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}.eps".format(index,tw,L,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Ave_overlap_J_hidden_{:s}_tw{:d}_L{:d}_N{:d}_beta{:2.0f}_step{:d}.pdf".format(index,tw,L,N,beta,tot_steps),format='pdf')
