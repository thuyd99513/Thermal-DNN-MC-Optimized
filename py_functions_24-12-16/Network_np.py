...
from utilities import calc_ener, calc_ener_batch, draw_history, random_numbers_with_gap
...
class Network:
    ...
    def update_J_np(self, J, n2, n1, x, EPS, rand, sqrt_num, pre_layer, next_layer):
        batch_size = n2.shape[0]
        for i in range(batch_size):
            self.update_J(J, n2[i], n1[i], x[i], EPS, rand[i], sqrt_num, pre_layer, next_layer[:, i])

    def update_J_np_batch(self, J, n2, n1, x, EPS, rand, sqrt_num, pre_layer, next_layer):
        batch_size = n2.shape[0]
        new_J = J[n2].copy()  # 使用 copy 避免直接修改原始数组

        # 更新指定列的值
        new_J[np.arange(batch_size), n1] = (J[n2, n1] + x * self.RAT) * self.RESCALE_J

        # 重缩放每一行
        norm = np.sqrt(np.sum(new_J ** 2, axis=1, keepdims=True))  # shape = (batch_size, 1)
        SCALE = np.sqrt(self.N / norm)  # shape = (batch_size, 1)
        new_J *= SCALE  # 按行缩放

        # 计算 a1 和 a2
        a1 = self.part_gap_common_shift_np(sqrt_num, J[n2], pre_layer, next_layer)  # shape = (batch_size, ...)
        a2 = self.part_gap_common_shift_np(sqrt_num, new_J, pre_layer, next_layer)  # shape = (batch_size, ...)

        # 计算能量变化 delta_e
        delta_e = calc_ener_batch(a2) - calc_ener_batch(a1)  # shape = (batch_size,)

        # 判断接受/拒绝更新
        accept_mask = (delta_e < EPS) | (rand < np.exp(-delta_e * self.beta))  # shape = (batch_size,)
    ...
    def update_tasks(self, n2, update_list, J, EPS, sqrt_num, pre_layer, next_layer):
        for n1, x, rand in update_list:
            self.update_J(J, n2, n1, x, EPS, rand, sqrt_num, pre_layer, next_layer)
    ...
    @timethis
    def random_update_J_batch_np(self, batch_size):
        EPS = self.EPS
        cutoff1 = self.cutoff1
        cutoff2 = self.cutoff2
        P1 = cutoff1 / self.num_bonds
        ...
        # 生成批量随机数
        x_batch = np.random.normal(loc=0.0, scale=1.0, size=batch_size)
        rand_batch = np.random.rand(batch_size)
        ...
        if np.any(mask_case1):
            true_count = np.sum(mask_case1)
            n1_case1 = np.random.randint(0, self.N_in, size=true_count)
            ...
            for i in range(epoch):
                ...
                self.update_J_np(self.J_in, n2, n1_for_n2, x_for_n2, EPS, rand_for_n2,
                                 self.SQRT_N_IN, self.S_in, self.S[:, 0, n2])

            remain = true_count % self.N
            if remain > 0:
                n2 = np.random.permutation(self.N)[:remain]
        ...
        if np.any(mask_case2):
            true_count = np.sum(mask_case2)
            n1_case2 = np.random.randint(0, self.N, size=true_count)
            x_selected = x_batch[mask_case2]
            rand_selected = rand_batch[mask_case2]
            epoch = true_count // self.N_out
            n2 = np.arange(self.N_out)
            ...
            for i in range(epoch):
                ...
                self.update_J_np(self.J_out, n2, n1_for_n2, x_for_n2, EPS, rand_for_n2,
                                 self.SQRT_N, self.S[:, -1, :], self.S_out[:, n2])

            remain = true_count % self.N_out
            if remain > 0:
                n2 = np.random.permutation(self.N_out)[:remain]
        ...
        # Case 3: 更新 J_hidden
        if np.any(mask_case3):
            true_count = np.sum(mask_case3)
            n1_case3 = np.random.randint(0, self.N, size=true_count)
            ...
            for i in range(epoch):
                for l_index in range(self.num_hidden_bond_layers):
                    ...
                    self.update_J_np(self.J_hidden[l_index], n2, n1_for_l_n2, x_for_l_n2, EPS, rand_for_l_n2,
                                     self.SQRT_N, self.S[:, l_index, :], self.S[:, l_index + 1, n2])

            remain = true_count % batch_size
            if remain > 0:
                epoch = remain // self.N
                l_range = np.random.permutation(self.num_hidden_bond_layers)[:epoch]
                ...
                if remain > 0:
                    l_index = np.random.randint(0, self.num_hidden_bond_layers)
                    n2 = np.random.permutation(self.N)[:remain]
    ...
    def set_vars(self):
        ...
        self.S_traj_hyperfine[0, :, :,
        :] = self.S  # Note that self.S_traj will independent of self.S from now on. This o.S is the state of S at the end of last epoch of training
        self.J_hidden_traj_hyperfine[0, :, :,
        :] = self.J_hidden  # This o.J is the state of J at the end of last epoch of training
    ...
    def accept_by_mu_l_n_delta(self, mu, l, n, delta):
        """This accept function is used if S is flipped."""
        self.S[mu, l, n] = -self.S[mu, l, n]
        self.H += delta
    ...
    def accept_by_mu_l_n_batch(self, mu_array, l_array, n_array, delta_H_array):
        ...
        batch_size = len(mu_array)
        for i in range(batch_size):
            self.accept_by_mu_l_n_delta(mu_array[i], l_array[i], n_array[i], delta_H_array[i])
    ...
    def part_gap_hidden_before_flip(self, mu, l_s, n):
        ...
        J_hidden_next = self.J_hidden[l_s, :, :] @ self.S[mu, l_s, :]  # Matrix-vector multiplication
    ...
    def part_gap_hidden_after_flip(self, mu, l_s, n):
        ...
        J_hidden_next = self.J_hidden[l_s, :, :] @ self.S[mu, l_s, :]  # 矩阵-向量乘法
    ...
    def part_gap_hidden_after_flip_batch_np(self, mu_batch, l_s_batch, n_batch):
        # 创建一个数组来存储所有批次的结果
        part_gap_batch = np.zeros((len(mu_batch), self.N + 1), dtype=np.float32)

        # 计算上一层的索引
        l_h_batch = l_s_batch - 1

        # 翻转自旋，使用高级索引同时更新所有 batch 的对应自旋
        ...
        # 计算 part_gap[0]，矩阵-向量乘法，进行批量操作
        part_gap_batch[:, 0] = (
                np.sum(self.J_hidden[l_h_batch, n_batch, :] * self.S[mu_batch, l_h_batch, :], axis=1)
                / self.SQRT_N
                * self.S[mu_batch, l_s_batch, n_batch]
        )

        # 计算 J_hidden_next_batch，矩阵-向量乘法，进行批量操作
        J_hidden_next_batch = np.einsum("bij,bj->bi", self.J_hidden[l_s_batch], S_l_batch)

        # 计算 part_gap 后面的部分
        part_gap_batch[:, 1:] = (J_hidden_next_batch / self.SQRT_N) * S_next_batch
    ...
    def part_gap_hidden_before_flip_batch_np(self, mu_batch, l_s_batch, n_batch):
        # 创建一个数组来存储所有批次的结果
        part_gap_batch = np.zeros((len(mu_batch), self.N + 1), dtype=np.float32)
    ...
    def part_gap_in_after_flip_batch_np(self, mu_vals, n_vals):
        # Preallocate the result array (part_gap)
        part_gap = np.zeros((mu_vals.shape[0], self.N + 1), dtype=np.float32)
        ...
        part_gap[:, 0] = (np.sum(self.J_in[n_vals, :] @ self.S_in[mu_vals, :].T / self.SQRT_N_IN, axis=1)
                          * self.S[mu_vals, 0, n_vals])

        # Compute the part_gap for indices 1 to N (next layer)
        part_gap[:, 1:] = (np.sum(self.J_hidden[0, :, :] @ self.S[mu_vals, 0, :].T / self.SQRT_N, axis=1)
                           * self.S[mu_vals, 1, :])

        # Restore the original sign of S[mu, 0, n]
        self.S[mu_vals, 0, n_vals] = -self.S[mu_vals, 0, n_vals]

        # Return the part_gap array (batch-wise result)
        return part_gap

    def part_gap_in_before_flip_batch_np(self, mu_vals, n_vals):
        # Preallocate the result array (part_gap)
        part_gap = np.zeros((mu_vals.shape[0], self.N + 1), dtype=np.float32)
    ...
    def part_gap_out_after_flip_batch_np(self, mu_vals, n_vals):
        # Preallocate the result array for batch results
        part_gap_batch = np.zeros((len(mu_vals), self.N_out + 1), dtype=np.float32)
        ...
        J_hidden_S = np.dot(self.J_hidden[-1, n_vals, :], self.S[mu_vals, -2, :].T)  # Dot product
        ...
        J_out_S_new = np.dot(self.J_out, self.S[mu_vals, -1, :].T)  # Matrix-vector multiplication
        J_out_S_new_expanded = J_out_S_new.T  # (N_out, len(mu_vals)) -> (len(mu_vals), N_out)
    ...
    def part_gap_out_before_flip_batch_np(self, mu_vals, n_vals):
        # Preallocate the result array for batch results
        part_gap_batch = np.zeros((len(mu_vals), self.N_out + 1), dtype=np.float32)
    ...
    def part_gap_common_shift_np(self, in_num, J_n, S_before, S_after):
        part_gap = (np.sum(J_n[:, np.newaxis, :] * S_before, axis=2) / in_num) * S_after.T
        return part_gap
    ...
    def decision_by_mu_l_n_batch(self, mu_array, l_array, n_array, EPS, rand_array):
        ...
        delta_H_array = (calc_ener_batch(self.part_gap_hidden_after_flip_batch(mu_array, l_array, n_array))
                         - calc_ener_batch(self.part_gap_hidden_after_flip_batch(mu_array, l_array, n_array)))
    ...
    def decision_node_in_by_mu_n_batch(self, mu_vals, n_vals, EPS, rand_vals):
        ...
        # 对不符合直接接受条件的样本计算概率接受条件
        not_accepted = ~accept_condition
        prob_ref_vals = np.exp(-delta_H_vals[not_accepted] * self.beta)
        accept_condition[not_accepted] = rand_vals[not_accepted] < prob_ref_vals

        # 处理所有符合条件的样本
        self.accept_by_mu_l_n_batch(
            mu_vals[accept_condition],
    ...
    def mc_random_update_hyperfine_2(self, str_timestamp):
        ...
        for mc_index in range(self.num // 189):
            self.mc_update_J_or_S_batch(MC_STEPS * 189)
            # Check and save the configureation of variables
