...
if __name__ == '__main__':
    ...
    ggrand_Q_J = np.zeros((len(alpha_list),len(tw_list),len(l_list)))
    ggrand_q_S = np.zeros((len(alpha_list),len(tw_list),len(l_S_list)))
    #==========================================================================
    ...
    for index_M,term in enumerate(M_list):
        ggrand_Q_J[index_M] = np.load(main_dir+'/ir_hf_L{:d}_M{:d}_N{:d}_sample{:d}_mp/data1/grand_Q_J_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(L,term,N,init,L,term,N,beta,tot_steps_list[index_M]))
        ggrand_q_S[index_M] = np.load(main_dir +'/ir_hf_L{:d}_M{:d}_N{:d}_sample{:d}_mp/data1/grand_q_S_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(L,term,N,init,L,term,N,beta,tot_steps_list[index_M]))
    ...
    np.save('{}/ggrand_Q_J_{:s}_L{:d}_N{:d}_beta{:4.2f}.npy'.format(data_dir,timestamp_list[i],L,N,beta),ggrand_Q_J)
    np.save('{}/ggrand_q_S_{:s}_L{:d}_N{:d}_beta{:4.2f}.npy'.format(data_dir,timestamp_list[i],L,N,beta),ggrand_q_S)
    ...
    ggrand_Q_J = ggrand_Q_J.transpose((1,2,0))
    ggrand_q_S = ggrand_q_S.transpose((1,2,0))
    for l_index,term in enumerate(l_list): 
        print("shape of ggrand_Q_J:")
        print(ggrand_Q_J.shape)
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[:],l_index,L,N,beta,alpha_list,tw_list[:])
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[0::2],l_index,L,N,beta,alpha_list,tw_list[0::2])
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[1::2],l_index,L,N,beta,alpha_list,tw_list[1::2])
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[0::3],l_index,L,N,beta,alpha_list,tw_list[0::3])
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[-2:-1],l_index,L,N,beta,alpha_list,tw_list[-2:-1])
        plot_ggrand_tau_J_tw_ave_over_init(ggrand_Q_J[-1:],l_index,L,N,beta,alpha_list,tw_list[-1:])
    for l_index,term in enumerate(l_S_list): 
        print("for l={}, shape of ggrand_q_S:".format(l_index))
        print(ggrand_q_S.shape)
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[:],l_index,L,N,beta,alpha_list,tw_list[:])
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[0::2],l_index,L,N,beta,alpha_list,tw_list[0::2])
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[1::2],l_index,L,N,beta,alpha_list,tw_list[1::2])
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[0::3],l_index,L,N,beta,alpha_list,tw_list[0::3])
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[-2:-1],l_index,L,N,beta,alpha_list,tw_list[-2:-1])
        plot_ggrand_tau_S_tw_ave_over_init(ggrand_q_S[-1:],l_index,L,N,beta,alpha_list,tw_list[-1:])
