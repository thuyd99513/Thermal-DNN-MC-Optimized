...
from Network import l_list, qq, QS_cutoff, QJ_cutoff, step_list_v2
...
if __name__ == '__main__':
    ...
    grand_tau_J_LOGIC = np.zeros((shape[0],shape[1]))
    grand_tau_S_LOGIC = np.zeros((shape2[0],shape2[1]))
    for tw_index in range(shape[0]):
        ...
        for l_index in range(shape[1]):
            ...
            if x1 > QJ_cutoff:
                grand_tau_J_LOGIC[tw_index,l_index] = 1 
                grand_Q_J[tw_index,l_index] = x1 
            else: 
                grand_tau_J_LOGIC[tw_index,l_index] = 0
                grand_Q_J[tw_index,l_index] = x1
                #grand_tau_J[tw_index,l_index] = IS_SOLID(QJ,step_list_v2,num,QJ_cutoff)
    ...
    for tw_index in range(shape2[0]):
        ...
        for l_index in range(shape2[1]):
            ...
            if x2 > QS_cutoff:
                grand_tau_S_LOGIC[tw_index,l_index] = 1 
                grand_q_S[tw_index,l_index] = x2
            else: 
                grand_tau_S_LOGIC[tw_index,l_index] = 0 
                grand_q_S[tw_index,l_index] = x2 
            #for Q
            print("grand_tau_S:")
            print(grand_q_S)
    ...
    print("grand_tau_S:")
    print(grand_q_S)
    np.save('{}/grand_tau_J_LOGIC_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,L,M,N,beta,tot_steps), grand_tau_J_LOGIC)
    np.save('{}/grand_tau_S_LOGIC_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,L,M,N,beta,tot_steps), grand_tau_S_LOGIC)
    ...
    grand_tau_J_LOGIC = np.zeros(shape[1])
    grand_tau_S_LOGIC = np.zeros(shape2[1])
    for l_index in range(shape[1]):
        print("l index:")
        print(l_index)
        ...
        x1 = np.mean(QJ[n_sampling_start:n_sampling_stop])
        if x1 > QJ_cutoff:
            grand_tau_J_LOGIC[l_index] = 1 
            grand_Q_J[l_index] = x1 
        else: 
            grand_tau_J_LOGIC[l_index] = 0
            grand_Q_J[l_index] = x1
    ...
    for l_index in range(shape2[1]):
        ...
        x2 = np.mean(QS[n_sampling_start:n_sampling_stop])
        if x2 > QS_cutoff:
            grand_tau_S_LOGIC[l_index] = 1 
            grand_q_S[l_index] = x2
        else: 
            grand_tau_S_LOGIC[l_index] = 0 
            grand_q_S[l_index] = x2 
        #for Q
        print("grand_tau_S:")
        print(grand_q_S)
    ...
    print("grand_tau_S:")
    print(grand_q_S)
    np.save('{}/grand_tau_J_LOGIC_mean_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,L,M,N,beta,tot_steps), grand_tau_J_LOGIC)
    np.save('{}/grand_tau_S_LOGIC_mean_L{:d}_M{:d}_N{:d}_beta{:4.2f}_step{:d}.npy'.format(data_dir,L,M,N,beta,tot_steps), grand_tau_S_LOGIC)
